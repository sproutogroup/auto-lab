...
describe("Business Logic Tests", () => {
 ...
 describe("Vehicle Management", () => {
  it("should calculate financial totals correctly", async () => {
   ...
   // Cleanup
   await request(app).delete(`/api/vehicles/${vehicle.id}`).set("Cookie", authCookie);
  });
 ...
 describe("Customer Management", () => {
  ...
  it("should reject duplicate email addresses", async () => {
   ...
   await request(app).post("/api/customers").set("Cookie", authCookie).send(customerData).expect(400);
  });
  ...
  afterAll(async () => {
   if (testCustomerId) {
    await request(app).delete(`/api/customers/${testCustomerId}`).set("Cookie", authCookie);
   }
  });
 });
 ...
 describe("Lead Management", () => {
  ...
  afterAll(async () => {
   if (testLeadId) {
    await request(app).delete(`/api/leads/${testLeadId}`).set("Cookie", authCookie);
   }
  });
 });
 ...
 describe("Data Validation", () => {
  it("should validate vehicle data before saving", async () => {
   ...
   await request(app).post("/api/vehicles").set("Cookie", authCookie).send(invalidVehicle).expect(400);
  });
  ...
  it("should validate customer email format", async () => {
   ...
   await request(app).post("/api/customers").set("Cookie", authCookie).send(invalidCustomer).expect(400);
  });
  ...
  it("should validate lead priority values", async () => {
   ...
   await request(app).post("/api/leads").set("Cookie", authCookie).send(invalidLead).expect(400);
  });
 });
});
