...
async function testServiceInitialization() {
  try {
    ...
    const { WebPushService } = await import(
...
async function testPushEncryption() {
  try {
    ...
    console.log(
      "✅ Push encryption test passed (endpoint invalid as expected)",
    );
...
async function evaluateOverallReadiness() {
  ...
  console.log(
    `   Readiness Score: ${readinessScore.toFixed(1)}% (${passedTests}/${totalTests} tests passed)`,
  );
}
