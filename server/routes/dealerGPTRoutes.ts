...
// Acknowledge insight
router.post(
  "/api/dealergpt/insights/:id/acknowledge",
  requireAuth,
  async (req, res) => {
    try {
      const userId = req.user?.id;
...
router.get("/api/dealergpt/memory", requireAuth, async (req, res) => {
  try {
    ...
    let memories;
    if (type) {
      memories = await aiMemoryService.getByType(
        type as string,
        parseInt(limit as string),
      );
    } else {
      memories = await aiMemoryService.getByUser(
        userId,
        parseInt(limit as string),
      );
    }
...
router.get("/api/dealergpt/memory/search", requireAuth, async (req, res) => {
  try {
    ...
    const results = await aiMemoryService.search(
      query,
      parseInt(limit as string),
    );
    res.json(results);
...
router.post("/api/dealergpt/memory", requireAuth, async (req, res) => {
  try {
    ...
    if (!key || !data || !memory_type) {
      return res
        .status(400)
        .json({ message: "Key, data, and memory_type are required" });
    }
...
router.post("/api/dealergpt/feedback", requireAuth, async (req, res) => {
  try {
    ...
    if (!conversationId || !feedback) {
      return res
        .status(400)
        .json({ message: "Conversation ID and feedback are required" });
    }
...
router.get("/api/dealergpt/data-status", requireAuth, async (req, res) => {
  try {
    const validatedData =
      await dealerGPTService.fetchDealershipDataWithValidation();
    res.json({
      status: "success",
