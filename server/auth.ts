...
export function setupAuth(app: Express) {
  ...
  try {
    ...
    console.warn(
      "Falling back to memory store - sessions will not persist across server restarts",
    );
  ...
  app.get("/api/auth/permissions", async (req, res) => {
    try {
      ...
      const transformedPermissions = permissions.map((permission) => ({
        ...
        access_level:
          permission.permission_level === "full_access"
            ? "full_access"
            : "view_only",
    ...
    try {
      ...
      const updatedUser = await db
        ...
        .set({
          ...
          first_name:
            first_name !== undefined ? first_name : existingUser[0].first_name,
          last_name:
            last_name !== undefined ? last_name : existingUser[0].last_name,
