...
export class DealerGPTDataAccessService {
  ...
  async fetchComprehensiveDealershipData(): Promise<ComprehensiveDealershipData> {
    console.log(
      "[DealerGPT DataAccess] Starting comprehensive data collection...",
    );
    ...
    try {
      ...
      const autolabVehicles = vehicles.filter(
        (v) => v.sales_status?.toLowerCase() === "autolab",
      );
      const awaitingDelivery = vehicles.filter(
        (v) => v.collection_status?.toLowerCase() === "awd",
      );
      ...
      const inventoryTurnover =
        soldVehicles.length / (stockVehicles.length || 1);
      ...
      const comprehensiveData: ComprehensiveDealershipData = {
        ...
        system_status: {
          ...
          total_records:
            vehicles.length + customers.length + leads.length + sales.length,
        ...
        financial: {
          ...
          profit_margins: {
            ...
            adjusted_margin:
              (totalAdjustedProfit / totalSalesRevenue) * 100 || 0,
          },
        ...
        customers: {
          ...
          active_customers: customers.filter(
            (c) => c.customer_type === "active",
          ),
          customer_segments: this.groupBy(customers, "customer_type"),
          lifetime_values: this.calculateCustomerLifetimeValues(
            customers,
            soldVehicles,
          ),
        ...
        leads: {
          ...
          active_leads: leads.filter(
            (l) => !["lost", "converted"].includes(l.pipeline_stage),
          ),
        ...
        service: {
          service_history: jobs.filter((j) =>
            ["service", "mot", "repair"].includes(j.job_type),
          ),
          parts_inventory: [], // Would need parts data
          warranty_claims: soldVehicles.filter((v) => v.warranty_costs > 0),
          service_revenue: jobs
            .filter((j) => j.actual_cost)
            .reduce((sum, j) => sum + j.actual_cost, 0),
          parts_revenue: soldVehicles.reduce(
            (sum, v) => sum + parseFloat(v.parts_cost?.toString() || "0"),
            0,
          ),
        },
        ...
        analytics: {
          kpis: {
            ...
            lifetime_customer_value: this.calculateAverageCustomerValue(
              customers,
              soldVehicles,
            ),
          ...
          alerts: {
            slow_moving_stock:
              stockAgeAnalytics?.stockDetails?.filter(
      ...
      console.log(
        `[DealerGPT DataAccess] Data collection completed in ${Date.now() - startTime}ms`,
      );
      return comprehensiveData;
    } catch (error) {
      console.error(
        "[DealerGPT DataAccess] Error collecting comprehensive data:",
        error,
      );
      throw error;
    }
  ...
  private groupPaymentMethods(vehicles: any[]): Record<string, number> {
    ...
    vehicles.forEach((v) => {
      if (v.cash_payment > 0)
        methods.cash += parseFloat(v.cash_payment.toString());
      if (v.finance_payment > 0)
        methods.finance += parseFloat(v.finance_payment.toString());
      if (v.bank_payment > 0)
        methods.bank_transfer += parseFloat(v.bank_payment.toString());
      if (v.px_value > 0)
        methods.part_exchange += parseFloat(v.px_value.toString());
    });
  ...
  private calculateAverageDaysToSell(vehicles: any[]): number {
    const vehiclesWithBothDates = vehicles.filter(
      (v) => v.purchase_invoice_date && v.sale_date,
    );
   ...
   */
  private calculateCustomerLifetimeValues(
    customers: any[],
    vehicles: any[],
  ): any[] {
    return customers
      .map((customer) => {
        const customerPurchases = vehicles.filter(
          (v) =>
            v.customer_name
              ?.toLowerCase()
              .includes(customer.first_name?.toLowerCase()) ||
            v.customer_name
              ?.toLowerCase()
              .includes(customer.last_name?.toLowerCase()),
        );
        ...
        return {
          ...
          average_purchase:
            customerPurchases.length > 0
              ? totalSpent / customerPurchases.length
              : 0,
          first_purchase: customerPurchases[0]?.sale_date,
          last_purchase:
            customerPurchases[customerPurchases.length - 1]?.sale_date,
        };
      })
   ...
   */
  private calculateAverageCustomerValue(
    customers: any[],
    vehicles: any[],
  ): number {
    const lifetimeValues = this.calculateCustomerLifetimeValues(
      customers,
      vehicles,
    );
    const totalValue = lifetimeValues.reduce(
      (sum, cv) => sum + cv.total_spent,
      0,
    );
  ...
  private calculateProfitTrend(vehicles: any[]): string {
    ...
    if (recentVehicles.length === 0 || olderVehicles.length === 0)
      return "stable";

    const recentAvgProfit =
      recentVehicles.reduce(
        (sum, v) => sum + parseFloat(v.total_gp?.toString() || "0"),
        0,
      ) / recentVehicles.length;

    const olderAvgProfit =
      olderVehicles.reduce(
        (sum, v) => sum + parseFloat(v.total_gp?.toString() || "0"),
        0,
  ...
  private calculateCustomerTrend(customers: any[]): string {
    ...
    if (recentCustomers.length > previousCustomers.length * 1.2)
      return "growing";
    if (recentCustomers.length < previousCustomers.length * 0.8)
      return "declining";
    return "stable";
  }
  ...
  async executeCustomQuery(query: string): Promise<any> {
    try {
      ...
      console.error(
        "[DealerGPT DataAccess] Error executing custom query:",
        error,
      );
      throw error;
    }
  ...
  async getDataSlice(entity: string, filters?: any): Promise<any> {
    ...
    try {
      switch (entity.toLowerCase()) {
        case "vehicles":
          return filters
            ? await this.filterVehicles(filters)
            : await storage.getVehicles();

        case "customers":
          return filters
            ? await this.filterCustomers(filters)
            : await storage.getCustomers();

        case "leads":
          return filters
            ? await this.filterLeads(filters)
            : await storage.getLeads();

        case "sales":
          return filters
            ? await this.filterSales(filters)
      ...
      console.error(
        `[DealerGPT DataAccess] Error fetching ${entity} data:`,
        error,
      );
      throw error;
    }
  ...
  private async filterVehicles(filters: any): Promise<any[]> {
    ...
    return vehicles.filter((v) => {
      if (
        filters.status &&
        v.sales_status?.toLowerCase() !== filters.status.toLowerCase()
      )
        return false;
      ...
      if (
        filters.min_price &&
        parseFloat(v.purchase_price_total?.toString() || "0") <
          filters.min_price
      )
        return false;
      if (
  ...
  private async filterCustomers(filters: any): Promise<any[]> {
    ...
    return customers.filter((c) => {
      ...
      if (filters.city && c.city?.toLowerCase() !== filters.city.toLowerCase())
        return false;
  ...
  private async filterLeads(filters: any): Promise<any[]> {
    ...
    return leads.filter((l) => {
      ...
      if (filters.assigned_to && l.assigned_to !== filters.assigned_to)
        return false;
  ...
  private async filterSales(filters: any): Promise<any[]> {
    ...
    return sales.filter((s) => {
      if (
        filters.start_date &&
        new Date(s.sale_date) < new Date(filters.start_date)
      )
        return false;
      if (
        filters.end_date &&
        new Date(s.sale_date) > new Date(filters.end_date)
      )
        return false;
      if (filters.salesperson_id && s.salesperson_id !== filters.salesperson_id)
        return false;
      return true;
    });
  }
