...
export class MobilePushService {
 ...
 async sendNotification(userId: number, payload: PushNotificationPayload): Promise<PushNotificationResult> {
  try {
   ...
   results.forEach((deviceResult, index) => {
    ...
    if (deviceResult.status === "fulfilled" && deviceResult.value) {
     ...
     result.errors.push({
      ...
      error:
       deviceResult.status === "rejected" ? deviceResult.reason?.message || "Unknown error" : "Send failed",
     });
    }
   });
 ...
 private async sendToDevice(device: DeviceRegistration, payload: PushNotificationPayload): Promise<boolean> {
 ...
 private async sendToIOS(device: DeviceRegistration, payload: PushNotificationPayload): Promise<boolean> {
 ...
 private async sendToAndroid(device: DeviceRegistration, payload: PushNotificationPayload): Promise<boolean> {
 ...
 private async sendToWeb(device: DeviceRegistration, payload: PushNotificationPayload): Promise<boolean> {
  try {
 ...
 async updateDevicePushSettings(deviceToken: string, pushEnabled: boolean): Promise<boolean> {
  try {
   const device = await storage.getDeviceRegistrationByToken(deviceToken);
 ...
 async getDeviceStats(userId?: number): Promise<{
  ...
  try {
   ...
   devices.forEach((device: any) => {
    ...
    if (
     device.last_active &&
     new Date(device.last_active) > new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000)
    ) {
 ...
 async testNotification(userId: number, platform: "ios" | "android" | "web"): Promise<boolean> {
