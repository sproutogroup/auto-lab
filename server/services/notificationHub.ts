...
export class NotificationHub {
 ...
 async sendNotification(
  ...
  try {
   ...
   const deliveryMethods = this.determineDeliveryMethods(userPreferences, options.deliveryMethods, template);
 ...
 async broadcastNotification(
  ...
  const results = await Promise.allSettled(
   userIds.map(userId => this.sendNotification(templateKey, userId, context, options)),
  );
