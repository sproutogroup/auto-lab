...
const RiskBadge = ({ risk }: { risk: string }) => {
  ...
  return (
    <Badge
      variant="outline"
      className={`${getRiskColor(risk)} text-xs font-medium`}
    >
  ...
  const filteredStockDetails = useMemo(() => {
    ...
    return analytics.stockDetails.filter((vehicle) => {
      ...
      const matchesAgeRange =
        ...
        (selectedAgeRange === "31-60" &&
          vehicle.days_in_stock > 30 &&
          vehicle.days_in_stock <= 60) ||
        (selectedAgeRange === "61-90" &&
          vehicle.days_in_stock > 60 &&
          vehicle.days_in_stock <= 90) ||
        (selectedAgeRange === "91-180" &&
          vehicle.days_in_stock > 90 &&
          vehicle.days_in_stock <= 180) ||
        (selectedAgeRange === "180+" && vehicle.days_in_stock > 180);

      const matchesRisk =
        selectedRisk === "all" || vehicle.depreciation_risk === selectedRisk;
            ...
            <div className="space-y-4">
              {analytics.makePerformance.slice(0, 6).map((make, index) => (
                <div key={index} className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <div className="text-sm font-semibold text-gray-900">
                      £{make.totalValue.toLocaleString()}
                    </div>
                    {make.slowMovingCount > 0 && (
                      <div className="text-xs text-red-600">
                        {make.slowMovingCount} slow moving
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  >
                    ...
                    <td className="py-3 px-2 text-center">
                      <div>
                        ...
                        <div className="text-xs text-gray-500">
                          £{vehicle.carrying_cost_daily}/day
                        </div>
                      </div>
                    </td>
                    <td className="py-3 px-2 text-center">
