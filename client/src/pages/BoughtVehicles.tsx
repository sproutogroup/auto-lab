...
function BoughtVehicleModal({
  vehicle,
  isOpen,
  onClose,
  mode,
  onEdit,
}: BoughtVehicleModalProps) {
  ...
  useEffect(() => {
    if (vehicle && isOpen) {
      form.reset({
        ...
        retail_price_1: vehicle.retail_price_1
          ? parseFloat(vehicle.retail_price_1.toString())
          : 0,
        retail_price_2: vehicle.retail_price_2
          ? parseFloat(vehicle.retail_price_2.toString())
          : 0,
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: BoughtVehicleForm) => {
      ...
      return apiRequest(
        "PUT",
        `/api/bought-vehicles/${vehicle.id}`,
        vehicleData,
      );
    },
    onSuccess: () => {
                      ...
                      {vehicle.retail_price_2 && (
                        <div>
                          ...
                          <p className="text-2xl font-bold text-green-600">
                            £
                            {parseFloat(
                              vehicle.retail_price_2.toString(),
                            ).toLocaleString()}
                          </p>
                        </div>
                      )}
                  ...
                  <img
                    ...
                    onError={(e) => {
                      console.error(
                        ...
                        vehicle.vehicle_images[selectedImageIndex],
                      );
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {mode === "create" &&
              "Add a new vehicle to the bought vehicles inventory"}
...
export default function BoughtVehicles() {
  const queryClient = useQueryClient();
  const [selectedVehicle, setSelectedVehicle] = useState<BoughtVehicle | null>(
    null,
  );
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  ...
                  {filteredVehicles
                    .reduce((sum, vehicle) => {
                      const value = vehicle.retail_price_1
                        ? parseFloat(vehicle.retail_price_1.toString())
                        : 0;
                      return sum + value;
                    }, 0)
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {
                    filteredVehicles.filter(
                      (vehicle) => vehicle.status === "AWAITING",
                    ).length
                  }
                </p>
              </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 mb-6">
            {searchTerm
              ...
              : "Get started by adding your first bought vehicle."}
          </p>
          <Button
            onClick={handleAddNew}
            className="bg-red-600 hover:bg-red-700"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredVehicles.map((vehicle: BoughtVehicle) => (
            <Card
              key={vehicle.id}
              className="premium-card hover:shadow-lg transition-shadow"
            >
                ...
                <div className="space-y-2">
                  ...
                  {vehicle.due_in && (
                    <div className="flex justify-between text-sm">
                      ...
                      <span>
                        {format(new Date(vehicle.due_in), "dd/MM/yyyy")}
                      </span>
                    </div>
                  )}
                </div>
