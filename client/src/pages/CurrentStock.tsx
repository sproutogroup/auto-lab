...
export default function CurrentStock() {
  ...
  const handleExport = () => {
    ...
    link.setAttribute(
      "download",
      `current_stock_export_${new Date().toISOString().split("T")[0]}.csv`,
    );
  ...
  const renderCellContent = (vehicle: VehicleData, column: any) => {
    ...
    if (
      (column.key.includes("purchase_") &&
        column.key !== "purchase_invoice_date") ||
      column.key === "vat"
    ) {
      return (
        <span className="font-mono">{formatCurrency(value as string)}</span>
      );
    }
                  ...
                  >
                    {columns.map((column) => (
                      <td
                        ...
                        title={
                          vehicle[
                            column.key as keyof VehicleData
                          ]?.toString() || ""
                        }
                      >
            ...
            >
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center space-x-2">
                  ...
                  <StatusBadge
                    value={vehicle.sales_status || ""}
                    type="sales"
                  />
                </div>
                <div className="flex items-center space-x-2">
                  <StatusBadge
                    value={vehicle.collection_status || ""}
                    type="collection"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div className="space-y-1">
                  ...
                  <div className="text-gray-900">
                    {vehicle.mileage
                      ? parseInt(vehicle.mileage.toString()).toLocaleString()
                      : "N/A"}
                  </div>
                </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="text-gray-900">
                      {vehicle.purchase_invoice_date
                        ? formatDate(vehicle.purchase_invoice_date)
                        : "N/A"}
                    </div>
                  </div>
