...
export default function VehicleMaster() {
  ...
  const importMutation = useMutation({
    mutationFn: async (csvData: any[]) => {
      console.log(
        `[Frontend] Starting CSV import with ${csvData.length} vehicles`,
      );
      ...
      if (line) {
        ...
        headers.forEach((header, index) => {
          ...
          const dbField =
            ...
            cleanHeader.toLowerCase().replace(/[^a-z0-9]/g, "_");
                    ...
                    {columns.map((column) => (
                      ...
                      >
                        {(() => {
                          ...
                          const value =
                            vehicle[column.key as keyof VehicleData];
                          ...
                          if (column.key === "mileage" && value) {
                            ...
                            return isNaN(mileageNum)
                              ? ""
                              : mileageNum.toLocaleString();
                          }
            ...
            {vehicles
              .reduce((sum, v) => {
                const price = parseFloat(
                  v.purchase_price_total?.replace(/[£,]/g, "") || "0",
                );
                return sum + price;
              }, 0)
