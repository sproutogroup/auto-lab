        ...
        <tbody>
         {sortedJobs.map((job: any) => (
          <tr key={job.id} className="border-b hover:bg-gray-50">
           ...
           <td className="p-2 text-xs capitalize">{job.job_type?.replace("_", " ")}</td>
           <td className="p-2">
           ...
           <td className="p-2 text-xs">£{job.actual_cost || job.estimated_cost || "0.00"}</td>
           <td className="p-2">
      ...
      <div className="space-y-4">
       <div className="flex justify-between items-center">
        ...
        <span className="font-semibold">
         {filteredJobs.length > 0 ? Math.round((completedJobs.length / filteredJobs.length) * 100) : 0}%
        </span>
       </div>
       <div className="w-full bg-gray-200 rounded-full h-2">
        <div
         ...
         style={{
          width: `${filteredJobs.length > 0 ? (completedJobs.length / filteredJobs.length) * 100 : 0}%`,
         }}
        ></div>
       </div>
       ...
       <div className="flex justify-between items-center">
        ...
        <span className="font-semibold">
         ...
         {filteredJobs.length > 0
          ? (
             filteredJobs.reduce(
              (sum: number, job: any) => sum + parseFloat(job.actual_cost || job.estimated_cost || "0"),
              0,
             ) / filteredJobs.length
            ).toFixed(2)
          : "0.00"}
        ...
        <span className="font-semibold">
         {filteredJobs.length > 0
          ? (
             filteredJobs.reduce(
              (sum: number, job: any) =>
               sum + parseFloat(job.actual_duration_hours || job.estimated_duration_hours || "0"),
              0,
             ) / filteredJobs.length
            ).toFixed(1)
          : "0.0"}
