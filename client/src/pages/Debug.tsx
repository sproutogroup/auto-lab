...
export default function DebugPage() {
  ...
  return (
   <div className="container mx-auto p-6">
    <Alert>
     ...
     <AlertDescription>Debug tools are not available in production.</AlertDescription>
    </Alert>
   </div>
  );
 }
 ...
 const generateSubscription = async () => {
  ...
  try {
   ...
   if (result.success) {
    ...
    addLog(`✓ Subscription registered with ID: ${result.subscription_id}`, "success");
   } else {
 ...
 return (
  <div className="container mx-auto p-6 max-w-4xl">
   <div className="mb-6">
    <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Debug - Push Notifications</h1>
   ...
   <div className="grid gap-6 md:grid-cols-2">
      ...
      {deviceInfo && (
       <div className="space-y-2 text-sm">
        ...
        <div className="flex justify-between">
         ...
         <Badge variant={deviceInfo.isIOS ? "default" : "secondary"}>{deviceInfo.isIOS ? "Yes" : "No"}</Badge>
        </div>
        <div className="flex justify-between">
    ...
    <Card>
     <CardHeader>
      ...
      <CardDescription>Current push subscription information</CardDescription>
     </CardHeader>
     <CardContent>
      {subscriptionId ? (
   ...
   <Card className="mt-6">
    <CardHeader>
     ...
     <CardDescription>Generate subscriptions and send test push notifications</CardDescription>
    </CardHeader>
    <CardContent>
     <div className="flex gap-4">
      <Button onClick={generateSubscription} disabled={isGenerating} className="flex items-center gap-2">
   ...
   <Card className="mt-6">
    <CardHeader>
     <CardTitle className="flex items-center justify-between">
      ...
      <Button onClick={clearLogs} variant="outline" size="sm" className="flex items-center gap-2">
      ...
      {logs.length === 0 ? (
       ...
       logs.map((log, index) => (
        <div key={index} className="mb-2 text-sm">
         ...
         <pre className="mt-1 text-xs whitespace-pre-wrap break-all">{log.message}</pre>
        </div>
       ))
      )}
     </div>
