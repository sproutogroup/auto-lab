...
export default function Lead_Modal({ isOpen, onClose, lead, mode }: LeadModalProps) {
 ...
 const stockVehicles = vehicles.filter(vehicle => vehicle.sales_status?.toLowerCase() === "stock");
 ...
 return (
  <Dialog open={isOpen} onOpenChange={onClose}>
   <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
    ...
    <Form {...form}>
     <form
      onSubmit={isViewMode ? e => e.preventDefault() : form.handleSubmit(onSubmit)}
      className="space-y-6"
     >
              ...
              {field.value && (
               <div className="flex items-center justify-between p-2 bg-blue-50 rounded-md border">
                <span className="text-sm text-blue-800">
                 Selected:{" "}
                 {stockVehicles.find(v => v.id === field.value)?.registration ||
                  stockVehicles.find(v => v.id === field.value)?.stock_number ||
                  "Unknown vehicle"}
                </span>
                {!isViewMode && (
                 <Button
       ...
       <TabsContent value="interactions" className="space-y-6">
        <Card>
         <CardHeader>
          ...
          <CardDescription>Track all communications and follow-ups with this lead</CardDescription>
         </CardHeader>
         <CardContent>
      ...
      <div className="flex justify-end gap-3 pt-6 border-t">
       <Button type="button" variant="outline" onClick={onClose} disabled={leadMutation.isPending}>
        Cancel
       </Button>
       {!isViewMode && (
        <Button type="submit" disabled={leadMutation.isPending} className="bg-red-600 hover:bg-red-700">
