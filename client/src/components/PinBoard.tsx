...
export function PinBoard() {
  ...
  const [editingMessage, setEditingMessage] = useState<PinnedMessage | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const unsubscribeCreated = subscribeToEvent(
    ...
    const unsubscribeUpdated = subscribeToEvent(
    ...
    const unsubscribeDeleted = subscribeToEvent(
  ...
  const getVisibilityText = (message: PinnedMessage) => {
    if (message.is_public) {
      ...
      const targetUsers = users.filter((u) =>
        message.target_user_ids?.includes(u.id),
      );
