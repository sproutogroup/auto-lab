      ...
      <div className="flex items-center gap-3">
       ...
       <div>
        <h2 className="text-2xl font-bold text-gray-900">
         {mode === "create" ? "Schedule New Job" : mode === "edit" ? "Edit Job" : "View Job"}
        </h2>
       </div>
      </div>
      {job && (
             ...
             {filteredVehicles.length > 0 ? (
              filteredVehicles.map((vehicle: any) => (
               ...
               >
                ...
                {selectedVehicle?.id === vehicle.id && <CheckCircle className="h-4 w-4 text-green-500" />}
               </div>
              ))
             ) : (
              <div className="p-3 text-gray-500 text-center">No vehicles found matching your search</div>
             )}
            </div>
           )}
          </div>
