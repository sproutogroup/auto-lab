...
export async function generateInvoicePdf(data: InvoiceApiData) {
  // ✅ CHANGE #1 — MUST be first (user gesture preserved)
  ...
  // 1) Load template PDF from /public
  const templateBytes = await fetch("/invoice-template.pdf", {
    cache: "no-store",
  ...
  //
  if (data.inspection_image_url) {
    const imageUrl = data.inspection_image_url;
  ...
  //
  // 5) Export — ✅ CHANGE #2 (safe download)
  //
  const filledBytes = await pdfDoc.save();
  const blob = new Blob([filledBytes as unknown as BlobPart], {
   type: "application/pdf",
