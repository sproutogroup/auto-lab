...
export default function Lead_Modal({
  ...
  const [should_close_on_success, set_should_close_on_success] =
    useState(false);
  const [is_interaction_in_progress, set_is_interaction_in_progress] =
    useState(false);
  ...
  const stock_vehicles = vehicles.filter(
      ...
      ? stock_vehicles.filter(
          (vehicle) =>
            vehicle.registration
              ?.toLowerCase()
              .includes(vehicle_search_query.toLowerCase()) ||
  ...
  const lead_mutation = useMutation({
    mutationFn: async (data: LeadFormData) => {
      ...
      const transformedData = {
        ...
        trade_in_value: data.trade_in_value
          ? Number(data.trade_in_value) || null
          : null,
        contact_attempts: 0,
    ...
    onSuccess: () => {
      ...
      if (should_close_on_success) {
        ...
        console.log(
          "Preventing modal close - interaction in progress or on interactions tab",
        );
      }
    },
    onError: (error: Error) => {
          ...
          <DialogHeader className="border-b pb-6 sm:pb-4">
            ...
            <div className="sm:hidden">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <Badge
                  className={`${getStageColor(lead.pipeline_stage || "new")} text-xs font-medium px-2 py-1`}
                >
                  {lead.pipeline_stage?.replace(/_/g, " ").toUpperCase() ||
                    "NEW"}
                </Badge>
                <Badge
                  className={`${getPriorityColor(lead.priority || "medium")} text-xs font-medium px-2 py-1`}
                >
            ...
            <div className="hidden sm:flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <Badge
                  className={`${getStageColor(lead.pipeline_stage || "new")} text-sm font-medium px-3 py-1`}
                >
                  {lead.pipeline_stage?.replace(/_/g, " ").toUpperCase() ||
                    "NEW"}
                </Badge>
                <Badge
                  className={`${getPriorityColor(lead.priority || "medium")} text-sm font-medium px-3 py-1`}
                >
                  {lead.priority?.toUpperCase() || "MEDIUM"}
                </Badge>
              </div>
                          ...
                          {assigned_vehicle ? (
                            <div className="mt-2 p-3 bg-white dark:bg-gray-800 rounded-lg border sm:p-2">
                              ...
                              <p className="text-sm text-gray-600 dark:text-gray-300 sm:text-xs">
                                {assigned_vehicle.make} {assigned_vehicle.model}{" "}
                                {assigned_vehicle.derivative}
                              </p>
                              <p className="text-sm text-gray-500 sm:text-xs">
                                {assigned_vehicle.year} •{" "}
                                {assigned_vehicle.colour} •{" "}
                                {assigned_vehicle.mileage?.toLocaleString()}{" "}
                                miles
                              </p>
                            </div>
                          ) : (
                            ...
                            <FormControl>
                              <div className="relative">
                                <Input
                                  ...
                                  onChange={(e) =>
                                    set_vehicle_search_query(e.target.value)
                                  }
                                  className="pr-10"
                                  disabled={is_view_mode}
                                />
