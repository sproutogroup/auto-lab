...
export default function DealerGPTAssistant() {
  ...
  const acknowledgeInsightMutation = useMutation({
    mutationFn: async (insightId: number) => {
      const response = await apiRequest(
        "POST",
        `/api/dealergpt/insights/${insightId}/acknowledge`,
        {},
      );
      return await response.json();
    },
    onSuccess: () => {
      ...
      setMessages((prev) =>
        prev.map((msg) => ({
          ...msg,
          proactiveAlerts: msg.proactiveAlerts?.filter(
            (alert) => alert.id !== acknowledgeInsightMutation.variables,
          ),
        })),
      );
    },
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        >
          ...
          <div className="flex-1 flex overflow-hidden">
            {/* Main Chat Area */}
            <div
              className={`flex-1 flex flex-col ${!isMinimized && showInsights ? "w-2/3" : "w-full"}`}
            >
              {/* Messages */}
              ...
              <div className="border-t p-4">
                <form onSubmit={handleSubmit} className="flex gap-2">
                  ...
                  <Button
                    type="submit"
                    disabled={
                      conversationMutation.isPending || !inputQuery.trim()
                    }
