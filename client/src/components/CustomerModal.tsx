          ...
          <Card className="shadow-lg border-0 bg-gradient-to-br from-neutral-50 to-neutral-100 dark:from-neutral-800/40 dark:to-neutral-700/40">
           ...
           <CardContent className="p-6 sm:p-4">
            <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border-l-4 border-neutral-500 sm:p-3">
             <p className="text-gray-900 dark:text-white sm:text-sm">{customer.notes}</p>
            </div>
           </CardContent>
          </Card>
         )}
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
     <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
      <DialogHeader>
       ...
       <DialogDescription>Update customer information and manage purchase history</DialogDescription>
      </DialogHeader>
                ...
                {purchase.finance_amount > 0 && (
                 <div className="mt-2 p-2 bg-blue-50 dark:bg-blue-900/20 rounded text-sm">
                  <div className="flex items-center gap-1">
                   ...
                   {purchase.finance_provider && (
                    <span className="text-gray-600 dark:text-gray-400">via {purchase.finance_provider}</span>
                   )}
                  </div>
                 </div>
                )}
   ...
   <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
    ...
    <Form {...purchaseForm}>
     <form onSubmit={purchaseForm.handleSubmit(onPurchaseSubmit)} className="space-y-6">
        ...
        {vehicleSearch && !selectedVehicle && (
         <div className="max-h-60 overflow-y-auto border rounded-lg">
          {filteredVehicles.length > 0 ? (
            ...
            >
             <div className="flex-1">
              ...
              <div className="text-sm text-gray-600 dark:text-gray-400">
               {vehicle.registration} • Stock: {vehicle.stock_number}
              </div>
             </div>
             <div className="text-right">
              <div className="font-medium">£{vehicle.total_sale_price?.toLocaleString() || "N/A"}</div>
              <Badge variant="outline" className="text-xs">
           ...
           <div className="p-4 text-center text-gray-500">No vehicles found matching your search</div>
          )}
         </div>
        )}
           ...
           <div className="flex items-center justify-between">
            <div>
             ...
             <p className="text-sm text-green-700 dark:text-green-400">
              {selectedVehicle.registration} • Stock: {selectedVehicle.stock_number}
             </p>
            </div>
            <div className="text-right">
         ...
         <FormField
          ...
          render={({ field }) => (
           <FormItem>
            ...
            <FormControl>
             <Textarea placeholder="Full delivery address..." className="min-h-[60px]" {...field} />
            </FormControl>
            <FormMessage />
           </FormItem>
          )}
      ...
      {/* Form Actions */}
      <div className="flex justify-end gap-3 pt-4 border-t">
       <Button type="button" variant="outline" onClick={onClose} disabled={createPurchaseMutation.isPending}>
        Cancel
       </Button>
       <Button
        type="submit"
